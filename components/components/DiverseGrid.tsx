...
export default function DiverseGrid({ products }: { products: Product[] }) {
  ...
  const handleVerMais = () => {
    ...
    // Entrada tipo "folhear livro": cada fileira nova surge de baixo,
    // como se fosse uma página a virar-se e a assentar — rotateX +
    // leve escala, coluna a coluna. Não mexe no scroll da página.
    requestAnimationFrame(() => {
      const newRows = rowRefs.current.slice(previousRowCount).filter(Boolean) as HTMLDivElement[];
      if (newRows.length === 0) return;

      gsap.set(newRows, { transformPerspective: 1000, transformOrigin: "top center" });

      const tl = gsap.timeline();
      newRows.forEach((rowEl, index) => {
        tl.fromTo(
          rowEl,
          { opacity: 0, y: 46, rotateX: -32, scale: 0.96 },
          {
            opacity: 1,
            y: 0,
            rotateX: 0,
            scale: 1,
            duration: DURATION_PER_COLUMN,
            ease: "power3.out",
          },
          index * (DURATION_PER_COLUMN * 0.5)
        );
      });
    });
  };
  ...
  const handleVerMenos = () => {
    ...
    orderedRows.forEach((rowEl, index) => {
      ...
      gsap.set(cards, { transformPerspective: 1000, transformOrigin: "top center" });
