...
export default async function Home() {
  const [siteSettings, destaquesMeta, vendidosMeta, novidadesMeta] = await Promise.all([
    getSiteSettings(),
    getSectionMeta("destaques"),
    getSectionMeta("vendidos"),
  ...
  const [destaques, vendidos, novidades] = await Promise.all([
    getSectionProducts("destaques", { orderBy: "created_at", ascending: false, from: 0, to: 9 }),
    getSectionProducts("vendidos", { orderBy: "vendas", ascending: false, from: 0, to: 9 }),
    getSectionProducts("novidades", { orderBy: "created_at", ascending: false, from: 10, to: 19 }),
  ]);

  const shownIds = [...destaques, ...vendidos, ...novidades].map((p) => p.id);
  ...
  return (
    <main className="bg-cacau-darker">
      ...
      <ProductRow
        title={vendidosMeta.title}
        eyebrow={vendidosMeta.eyebrow}
        viewAllHref="/secao/vendidos"
        products={vendidos}
      />
